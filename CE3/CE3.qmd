...
# Align predictions with data (skip first row due to lag)
Pred_vals_aligned <- Pred[[1]]$state$pred$Ti[-1]

# Remove first row (NA in naive)
df_resid_plot <- AllDat %>% 
  slice(-1) %>% 
  mutate(
    Resid_Model = Pred_vals_aligned - yTi4,
...
df_long <- df_resid_plot %>%
  ...
  mutate(Predictor = factor(Predictor, levels = c("Resid_Model", "Resid_Naive"),
...
# Faceted plot stacked
ggplot(df_long, aes(x = date, y = Residual)) +
  geom_line(aes(color = Predictor), size = 1) +
  geom_hline(yintercept = 0, linetype = "dotted") +
  scale_color_manual(values = c("Model Residual" = "#1f77b4", "Naive Residual" = "#ff7f0e")) +
