sdeTiTm_heatfrac <- function(data, yTi,Ph){
  data$yTi <- yTi
  data$Ph <- Ph
  ...
  model$addSystem(dTi ~  1/Ci*(1/Ria*(Ta-Ti) + 1/Rim*(Tm-Ti)  + frac*Ph + 
                                 Gv*( a1*bs1 + a2*bs2 + a3*bs3 + a4*bs4 + a5*bs5 ) )*dt 
                  + exp(p11)*dw1)
  model$addSystem(dTm ~  1/Cm*(1/Rim*(Ti-Tm) + (1-frac)*Ph)*dt + exp(p22)*dw2)
  ...
  model$setParameter(frac = c(init = 1/2, lb = 1E-2, ub = 0.99))
