sde4room_simple <- function(data){
  ...
  model$addSystem(
    dTi1 ~ 1/Ci1*( 1/Ria*(Ta-Ti1) + 1/Rim*(Tm1-Ti1) + 1/R12*(Ti1-Ti2) + Ph1 + Aw1*Gv)*dt 
    + exp(logsigma_i)*dw1
  )
  model$addSystem(
  ...
  model$addSystem(
    dTi2 ~ 1/Ci2*( 1/Ria*(Ta-Ti2) + 1/Rim*(Tm2-Ti2) + 1/R12*(Ti1-Ti2) + 1/R23*(Ti2-Ti3) + Ph1 + Aw2*Gv)*dt 
    + exp(logsigma_i)*dw3
  )
  model$addSystem(
  ...
  model$addSystem(
    dTi3 ~ 1/Ci3*( 1/Ria*(Ta-Ti3) + 1/Rim*(Tm3-Ti3) + 1/R23*(Ti2-Ti3) + 1/R34*(Ti3-Ti4) + Ph2 + Aw34*Gv)*dt 
    + exp(logsigma_i)*dw5
  )
  model$addSystem(
  ...
  model$addSystem(
    dTi4 ~ 1/Ci4*( 1/Ria*(Ta-Ti4) + 1/Rim*(Tm4-Ti4) + 1/R34*(Ti3-Ti4) + Ph2 + Aw34*Gv)*dt 
    + exp(logsigma_i)*dw7
  )
  model$addSystem(
  ...
  model$setParameter(Cm2 = c(init = 1000, lb = 1E-5, ub = 2E5))
  ...
  model$setParameter(Aw34 = c(init = 6, lb = 1E-2, ub = 7.5+4.8+5))
