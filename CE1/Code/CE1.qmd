...
a = min(y)
b = max(y)
Gamma1 <- 3 * (b-a) + 0.8/2 * (b^2 - a^2)

b <- seq(-2, 2, length.out = 500)
lambda <- ifelse(b < 0,
                 1.6 - 0.4 * b^2,
                 1.6 + 0.4 * b^2)
...
# Find common y-range
ylim_all <- range(c(lambda, Lambda, Lambda.lower, Lambda.upper))

# Empty plot frame
plot(NA, xlim = range(tail(breaks, -1), ), ylim = ylim_all,
     xlab = "b / index", ylab = expression(Lambda),
     type = "n")

# Parametric Lambda(b)
lines(tail(breaks, -1),  lambda, col = "blue", lwd = 2)
abline(h = 1.6, lty = 2, col = "gray")

# Empirical Lambda with confidence bands
lines(tail(breaks, -1), Lambda, col = "black", lwd = 2)
lines(tail(breaks, -1), Lambda.lower, col = "red", lty = 2)
lines(tail(breaks, -1), Lambda.upper, col = "red", lty = 2)
...
plot(res[1:(n-2)], res[3:n]) # Plotting residuals

plot(x[1:(n-2)], x[3:n]) # Plotting x against x_{n-2}
abline(v = c(-4.2, -0.19, 2.8), col = "red")
...
#legend("bottomleft",
#       legend = c("x[t-2]< -4.2", "x2: [c1,c2)", "x3: [c2,c3)", "x4: ≥c3"),
