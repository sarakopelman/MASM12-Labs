...
# Plot
plot(x, M, type="l", lwd=2, col="blue",
     xlab="x", ylab=expression(M(x)),
     main="Conditional Mean (Drift)")

abline(h=0, lty=2, col="gray")  # zero reference
abline(v=0, lty=2, col="gray")  # breakpoint at 0

a <- -8
b <- seq(-8, 8, length.out = 500)

lambda <- ifelse(b <= 0,
                 3*(b - a) + 0.4*(b^2 - a^2),
                 (3*(0 - a) + 0.4*(0^2 - a^2)) + (-3*(b - 0) - 0.4*(b^2 - 0^2))
)

plot(b, lambda, type="l", lwd=2, col="blue",
...
x <- y
##----------------------------------------------------------------
## Script for calculation of confidence bands using the cumulative means technique.
## First generate x: a time series with a realization of a Markov process.
...
## Calc the histogram regressogram
for(i in 1:n.bin) {
  x.bin <- L[[i]]
  lambda[i] <- mean(x.bin)
...
## Make confidence bands for the cumulated function
c.alpha <- 1.273
...
# Define parametric curve using the same bin centers
b <- head(breaks, -1) + h/2   # bin centers
a <- min(b)

lambda_param <- ifelse(b <= 0,
                       3 * (b - a) + 0.4 * (b^2 - a^2),
                       (3 * (0 - a) + 0.4 * (0^2 - a^2)) + (-3 * (b - 0) - 0.4 * (b^2 - 0^2))
)

# Find common y-range
ylim_all <- range(c(lambda_param, Lambda, Lambda.lower, Lambda.upper))
...
abline(h = 1.6, lty = 2, col = "gray")

# Empirical Lambda with confidence bands
