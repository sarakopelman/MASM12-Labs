...
P = matrix(c(0.9, 0.1, 0.7, 0.3), nrow = 2, ncol = 2, byrow = TRUE)
# Create state vector to save states
state <- integer(n)
state[1] <- 1
...
plot(1:300, y[1:300], type = "l", xlab = "Time", ylab = "Xt", main = "Simulated MMAR(2,2)")
plot(1:300, state[1:300], type = "l", xlab = "Time", ylab = "State" , main = "Markov Chain")
```
...
# -------------------------
cumulative_mean <- function(x, n.bin = 19){
  a <- min(x)
  b_max <- max(x)
  ...
  # Bin centers
  b <- head(breaks, -1) + h/2
  list(b = b, Lambda = Lambda, Lambda.lower = Lambda.lower, Lambda.upper = Lambda.upper, a = a, n.bin = n.bin)
}

# -------------------------
# 3️⃣ Theoretical cumulative mean
# -------------------------
theoretical_lambda <- function(b, a){
  ...
  for(i in seq_along(b)){
    ...
    if(bi < 0){
      ...
      left_part  <- 3*(0 - a) + 0.4*(0^2 - a^2)
      right_part <- -3*(bi - 0) - 0.4*(bi^2 - 0^2)
      Lambda_theor[i] <- left_part + right_part
    } else {
      Lambda_theor[i] <- -3*(bi - a) - 0.4*(bi^2 - a^2)
    }
  }
...
# -------------------------
# 4️⃣ Sample sizes and bin counts
# -------------------------
n_list <- c(1000, 5000, 10000)
n_bin_list <- c(19, 19, 19)  # one bin count per sample size
colors <- c("blue", "red", "green")

# -------------------------
# 5️⃣ Plot as subplots
# -------------------------
par(mfrow = c(1, length(n_list)), mar = c(4,4,3,1))  # 1 row, 3 columns

for(i in seq_along(n_list)){
  x <- simulate_setar(n_list[i])
  res <- cumulative_mean(x, n.bin = n_bin_list[i])  # use corresponding bin size
  lambda_theor <- theoretical_lambda(res$b, res$a)
  
  # Set limits consistently
  ylim_all <- range(c(res$Lambda, res$Lambda.lower, res$Lambda.upper, lambda_theor))
  
  plot(res$b, res$Lambda, type = "l", col = colors[i], lwd = 2,
       ylim = ylim_all,
       xlab = expression(X[t-1]), ylab = "Cumulative mean",
       main = paste("n =", n_list[i]))
  lines(res$b, res$Lambda.lower, col = colors[i], lty = 2)
  lines(res$b, res$Lambda.upper, col = colors[i], lty = 2)
  lines(res$b, lambda_theor, col = "black", lty = 3, lwd = 1.5)
  
  legend("topright",
         legend = c("Empirical", "Lower/Upper CI", "Theoretical"),
         col = c(colors[i], colors[i], "black"),
         lwd = c(2,1,1.5), lty = c(1,2,3), bty = "n", cex = 0.8)
}
...
# -------------------------
# 2️⃣ Cumulative means function (central region)
# -------------------------
cumulative_mean_central <- function(x, n.bin = 51, x_range = c(-2,2), min_points = 5){
  # Only keep points in the central region
  x_cond <- x[-length(x)]
  x_next <- x[-1]
  idx <- which(x_cond >= x_range[1] & x_cond <= x_range[2])
  x_cond <- x_cond[idx]
  x_next <- x_next[idx]
  
  # Create bins
  breaks <- seq(x_range[1], x_range[2], length.out = n.bin + 1)
  h <- diff(breaks)[1]
  
  # Split data into bins
  L <- split(x_next, cut(x_cond, breaks))
  ...
  for(i in 1:n.bin){
    ...
    # Warning if bin has too few points
    if(length(x.bin) < min_points){
      warning(sprintf("Bin %d has less than %d points (%d points)", i, min_points, length(x.bin)))
    }
    
    lambda[i] <- mean(x.bin, na.rm = TRUE)
    f.hat[i] <- length(x.bin) / (n.bin * h)
    gamma[i] <- if(length(x.bin) > 0) sum((x.bin - lambda[i])^2, na.rm = TRUE) / length(x.bin) else 0
  }
  
  # Cumulative mean and confidence bands
  Lambda <- cumsum(lambda * h)
  for(i in 1:n.bin) h.hat[i] <- gamma[i] / f.hat[i]
  H.hat <- cumsum(h.hat * h)
  H.hat.b <- H.hat[n.bin]
  ...
  b <- head(breaks, -1) + h/2
  list(b = b, Lambda = Lambda, Lambda.lower = Lambda.lower, Lambda.upper = Lambda.upper)
}
...
# -------------------------
# 4️⃣ Compare different bin sizes (same large sample)
# -------------------------
n_large <- 10 0000
x_range <- c(-2, 2)
x <- simulate_setar(n_large)
bin_sizes <- c(23, 50, 100, 300)

# Compute results for each bin size
res_list <- lapply(bin_sizes, function(nb) cumulative_mean_central(x, n.bin = nb, x_range = x_range, min_points = 5))

# Determine consistent y-limits across plots
ylim_all <- range(unlist(lapply(res_list, function(r) c(r$Lambda.lower, r$Lambda.upper))))

# Plot results
par(mfrow = c(2, 2), mar = c(4, 4, 3, 1))
for(i in seq_along(bin_sizes)){
  res <- res_list[[i]]
  plot(NA, xlim = x_range, ylim = ylim_all,
       xlab = expression(X[t-1]), ylab = "Cumulative mean", type = "n",
       main = paste("n.bin =", bin_sizes[i]))
  
  polygon(c(res$b, rev(res$b)),
          c(res$Lambda.lower, rev(res$Lambda.upper)),
          col = adjustcolor("blue", alpha.f = 0.2),
          border = NA)
  
  lines(res$b, res$Lambda, col = "blue", lwd = 2)
  
  b_theor <- seq(x_range[1], x_range[2], length.out = 101)
  lines(b_theor, theoretical_lambda(b_theor, x_range[1]), col = "black", lwd = 2, lty = 2)
  
  legend("topright", legend = c("Empirical", "Theoretical"),
         col = c("blue","black"), lwd = 2, lty = c(1,2), bty = "n")
}
